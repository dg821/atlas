...
Eigen::Vector3d PerturbationMethod::computeTotalAcceleration(const double t, SpaceVehicle& sv) const {
    Eigen::Vector3d totalAcceleration = Eigen::Vector3d::Zero();
    for (auto forceModel : forceModels) {
        totalAcceleration = totalAcceleration + forceModel->computeAcceleration(t, sv);
    }
    return totalAcceleration;
}


Eigen::Vector3d PerturbationMethod::computePerturbAcceleration(double t, SpaceVehicle& sv) const {
    Eigen::Vector3d totalAcceleration = Eigen::Vector3d::Zero();
...
PerturbationMethod::Vector6d Cowell::equationsOfMotion(const double t, const Vector6d& stateVector, SpaceVehicle& sv) const {
    ...
    Eigen::Vector3d totalAcceleration = computeTotalAcceleration(t, sv);
...
void Cowell::updateState(SpaceVehicle& sv, const double t) {
    ...
    Eigen::Vector3d new_r = {stateVector(0), stateVector(1), stateVector(2)};
    Eigen::Vector3d new_v = {stateVector(3), stateVector(4), stateVector(5)};
...
void Encke::updateState(SpaceVehicle& sv, double t) {
    ...
    while (tNow < tEnd) {
        const auto [r_twoBody, v_twoBody] = keplerProblem::solveKepler(r, v, tNow);
...
void VariationOfParameters::updateState(SpaceVehicle& sv, double t) {
    ...
    if (ecc < 0.001 || inc < 0.001) {
        stateConversion::EquinoctialElements equin = stateConversion::Kep2Equinoctial(sma, ecc, inc, node, argP, truA);
        ...
        stateConversion::KeplerianElements kep = stateConversion::Equinoctial2Kep(stateVector(0), stateVector(1), stateVector(2), stateVector(3), stateVector(4), stateVector(5));
        stateConversion::CartesianState cart = stateConversion::Kep2Cart(kep.sma, kep.ecc, kep.inc, kep.node, kep.argP, kep.truA);
    ...
    tStart, tEnd, stateVector, dt, dt_min, dt_max
        ...
        stateConversion::CartesianState cart = stateConversion::Kep2Cart(stateVector(0), stateVector(1), stateVector(2), stateVector(3), stateVector(4), stateVector(5));
        Eigen::Vector3d new_r = cart.r;
        Eigen::Vector3d new_v = cart.v;
